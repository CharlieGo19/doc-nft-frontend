        ...
        <div class="w-full flex flex-col fixed z-50 bg-white/60 backdrop-blur-lg">
            <div class="flex flex-row">
                ...
                <div class="relative w-1/6 ml-auto mr-0">
                    <button type="button" class="w-full float-right" on:click={() => toggleMenu('mobile')}>
                        <div class="relative flex h-10 items-center justify-center">
                            <span class="absolute h-1 w-1/3 bg-mint-edge rounded-md -translate-y-2 {showMobileMenu === true ? 'opacity-0':''} transition delay-200 duration-300"></span>
                            <span class="absolute h-1 w-1/3 bg-mint-edge rounded-md {showMobileMenu === true ? '-rotate-45': ''} transform delay-200 duration-300"></span>
                            <span class="absolute h-1 w-1/3 bg-mint-edge rounded-md {showMobileMenu === true ? 'rotate-45': ''} transform delay-200 duration-300"></span>
                            <span class="absolute h-1 w-1/3 bg-mint-edge rounded-md translate-y-2 {showMobileMenu === true ? 'opacity-0':''} transition delay-200 duration-300}"></span>
                        </div>
                    </button>
            ...
            <div class="flex flex-row">
                {#if showMobileMenu}
                <div class="grid grid-cols-2 w-full text-black pt-1 pb-1" transition:slide={{ delay: 200, duration: 300, easing: linear }}>
                    <span class="p-2 hover:font-bold hover:bg-mint-edge hover:text-white">{$nftMarketPlace}</span>
                    <span class="p-2 hover:font-bold hover:bg-mint-edge hover:text-white">Launch Pad</span>
                    <span class="p-2 hover:font-bold hover:bg-mint-edge hover:text-white">Explore</span>
                    <span class="p-2 hover:font-bold hover:bg-mint-edge hover:text-white">Activity</span>
                    <span class="p-2 hover:font-bold hover:bg-mint-edge hover:text-white">DAO</span>
                    <span class="p-2 hover:font-bold hover:bg-mint-edge hover:text-white">Raffle</span>
                    <span class="p-2 hover:font-bold hover:bg-mint-edge hover:text-white">Account</span> <!-- TODO: Menu within a menu -->
                    <span class="p-2 hover:font-bold hover:bg-mint-edge hover:text-white">Wallet</span>
                </div>
                {/if}
            </div>
        </div>
