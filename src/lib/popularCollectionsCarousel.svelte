...
<div class="relative flex flex-row w-full h-[360px] md:max-w-7xl z-0">
    <button class="relative flex justify-center items-center select-none mt-[190px] mb-auto ml-2 sm:visible invisible bg-white z-20 rounded-md border-2 border-black" on:click={() => {scrollLeft(804, carousel);}}>
    ...
    <ul bind:this={carousel} class="absolute flex w-full z-10 mt-10 no-scrollbar overflow-x-auto gap-7 snap-x snap-mandatory scroll-smooth 
        ...
        {#each popularCollections as {name, verified, itemOneSrc, itemTwoSrc, itemThreeSrc, size}}
            ...
            <div class="flex flex-row h-5/6 w-full p-2">
                <div class="w-1/2">
                    <img src="{itemOneSrc}" alt="Collection: {name}" class="h-full rounded-xl object-cover" />
                </div>
                <div class="flex flex-col w-1/2 pl-1">
                    <img src="{itemTwoSrc}" alt="Collection: {name}" class="relative h-[122px] rounded-xl object-cover" />
                    <img src="{itemThreeSrc}" alt="Collection: {name}" class="relative h-[122px] mt-[4px] rounded-xl object-cover" />
                </div>
            </div>
    ...
    <button class="absolute flex justify-center items-center select-none right-0 mt-[190px] mr-2 sm:visible invisible bg-white z-20 rounded-md border-2 border-black" on:click={() => {scrollRight(804, carousel);}}>
