<script lang="ts">
    import {
        ERROR_GENERATING_QRCODE,
        HASHPACK_PAIRING_STRING_COPIED,
        HASHPACK_WALLET,
        TOAST_LEVEL_ERROR,
    ...
    onMount(async () => {
        ...
        try {
            genQrCode = await QRCode.toDataURL(
                $hashpackPairingString as string
            );
        } catch (e) {
            const t: Toast = {
...
<div
    class="w-full md:max-w-md flex flex-col translate-y-1/4 md:mt-auto md:mb-auto md:-translate-y-1/4 text-white"
>
    <span class="w-1/2 mx-auto py-1 text-center font-extrabold"
        >Pair Wallet</span
    >
    <button
        class="flex flex-row mx-auto py-1 justify-center border-4 border-[#525298] w-1/2 rounded-xl font-bold text-white hover:bg-black/50"
        on:click={pairWithExtension}
    >
        <img src="/wallets/hashpack-clear.svg" alt="HashPack" class="w-6 h-6" />
        <span class="px-4">EXTENSION</span>
    </button>
    <span class="w-1/2 mx-auto mt-4 py-1 text-center font-extrabold"
        >Pairing String</span
    >
    <div
        class="flex flex-row w-1/2 mx-auto px-3 py-3 space-x-2 border-4 border-[#525298] rounded-xl"
    >
        <input
            class="flex truncate bg-transparent focus:outline-none focus:border-none focus:ring focus:ring-white focus:ring-opacity-40 rounded-sm"
            value={$hashpackPairingString}
        />
        <button
            on:click={() => {
                copyToClipBoard(
    ...
    <span class="w-1/2 mx-auto mt-4 py-1 text-center font-extrabold"
        >Scan QR Code in your {HASHPACK_WALLET} app</span
    >
    <img
        src={genQrCode}
        alt="QRCode to pait with HashPack!"
        class="w-1/2 mx-auto"
    />
</div>
